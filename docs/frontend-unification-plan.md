...
用户确定的目标：营销、客服、运营 AI IM；前端由一个工程统一维护与管理，后端保持模块化。本文定义前端统一工程的终态和迁移顺序，承接 [product.md](product.md) 的业务方向；总工程见 [development.md](development.md)。
...
客服业务执行器与真实回执迁移属于 [development.md](development.md) 的 P1/P2。统一前端可以先适配旧 API，但在后端完成前不得将它宣传为可靠的自动营销执行平台。
...
- 统一前端已切到 `/`。旧壳在 `/legacy`。`/next` 重定向到对应的根路径。根路径的旧 Service Worker 在新前端启动时注销。
