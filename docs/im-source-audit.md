...
2026-09-14 strict compilation update: the source inventory is now 127 files, including the product-owned `src/env.d.ts`, ten JSON card renderers, and `UserSystemNotice.vue`. The unified TypeScript and Vite builds pass after native aliases, actual dependency typings, provider mapping result types, optional-value guards and component event contracts were repaired. The manifest preserves original extracted hashes for modified files and records the pinned baseline hash for added source. Mapping and SSR card tests cover recent contacts, absent file results, malformed JSONPath, account placeholders, valid miniapps, invalid music payloads, and unknown card IDs. These checks do not verify account isolation or real QQ sends; Connector/Pinia scope and chat-view runtime integration remain open gates.
