...
1. 接入原生 IM 的宿主、会话切换和历史控制器，提取入口依赖的基础 CSS；完成主题、弹层和窄屏验收。
2. 完成账号切换时的异步消息处理、延时任务与本地设置隔离。JSON 卡片闭包、传输门面和存储清理已经合入，不能将这部分验收当成完整原生 IM 已接入。
3. 在同一应用中安装并验证原生 IM，再完成 IM/Agent 普通双区域布局、主题与弹层隔离。`ImModule.vue` 当前仍是简化预览，不以源码构建成功替代接入验收。
...
尚未完成：`ImModule.vue` 仍为简化预览，原生 UserChat/UserMessages 尚未挂入路由；入口所需的基础 CSS 还需提取。原生消息处理器中已经开始执行的异步解析、延时回调，以及账号相关 localStorage 设置仍须加入账号作用域检查。不能只靠 Connector 的响应隔离证明这些后续任务已经隔离。真实账号收信、完整历史、媒体发送与 Agent 实际协议验收继续保留为门槛。
