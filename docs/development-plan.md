...
日期：2026-09-14。目标分支：develop。执行方式：按依赖拆分可审查的垂直切片，在独立 worktree 并行开发，由根任务逐项验收、合入和回收。
...
- P3 验收：补齐原生 IM 的媒体、历史、群资料和系统通知 fixture 回归；使用 gpt-5.6-terra。
- P6 前端：在统一前端实现账号隔离的客户画像、意图和证据工作台；使用 gpt-5.6-terra。
- P6 后端：建立跨群消息采集、可追溯聚合报告和覆盖范围契约；使用 gpt-5.6-sol。
- P7 发布：维护版本组合 manifest、备份/恢复/回滚脚本与 Compose 验收；使用 gpt-5.5。
