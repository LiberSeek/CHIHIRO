...
import { ref } from 'vue'
import { addMusic } from '@renderer/components/user/UserMusicPlayer.vue'
...
if (success) {
    getForegroundToneGridFromImageUrl(backend.proxyUrl(parsedContent.img), 0.4)
        .then(tone => { lightColor.value = tone[1][1] === 'light' })
        .catch(error => new Logger().error(error, 'Card cover unavailable'))
}
