...
import appInfo from '@renderer/package-info'
...
export async function loadAppendStyle() {
    ...
    // napcat 插件模式附加样式：嵌在 NapCat WebUI 里才需要玻璃透明。
    // 千寻是独立 iframe，加载这些样式会把卡片/顶栏/输入框洗成半透明，看起来像 CSS 缺失。
    if (import.meta.env.VITE_NAPCAT && !import.meta.env.VITE_CHIHIRO) {
        import('@renderer/assets/css/append/append_full_vibrancy.css').then(() => {
            logger.info('完全透明 UI 附加样式加载完成')
        })
...
export function checkOpenTimes() {
    ...
    const { $t } = app.config.globalProperties
    const repoName = import.meta.env.VITE_APP_REPO_NAME
    const times = null // Chihiro: skip open-count nags
    if (times != null) {
        const getTimes = Number(times) + 1
        localStorage.setItem('times', getTimes.toString())
        if (false) { // Chihiro: skip GitHub star nag
            // 构建 HTML
            let html =
        ...
        if (false) { // Chihiro: skip sponsor nag
            const popInfo = {
                title: '',
    ...
    // 使用引导：千寻跳过 WELCOME 向导，沿用 overlay 默认配置。
...
export function sendStatEvent(event: string, data: { [key: string]: any }, saveLocal = false) {
    ...
    if (false && !option.get('close_ga') && !import.meta.env.DEV) {
