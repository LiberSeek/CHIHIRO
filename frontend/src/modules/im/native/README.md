...
The extraction is intentionally not presented as feature parity. `src/` retains the real message renderers, composer, history, stores, connector, and supporting settings/info code reached by static imports. The only source rewrite replaces imports from upstream `main.ts` with `host.ts`, so importing the module cannot create or mount a second Vue application. The product must call `configureNativeImHost` before rendering.
...
The verifier checks the complete file list, hashes, and absence of upstream bootstrap imports or `createApp` calls. Regenerate the closure only from the pinned reference tree and review every transform; do not silently edit generated source without updating the manifest and audit.
