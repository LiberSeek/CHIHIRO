...
export const backupApi = {
  ...
  downloadUrl(filename: string, token: string) {
    return `${apiPrefix()}/backups/${encodeURIComponent(filename)}?token=${encodeURIComponent(isHosted() ? '' : token)}`;
  },
};
...
export const chatApi = {
  ...
  sendStreamUrl() {
    return `${apiPrefix()}/chat`;
  },
  resumeRunStreamUrl(runId: string) {
    return `${apiPrefix()}/chat/runs/${encodeURIComponent(runId)}/stream`;
  },
  liveWebSocketUrl(token: string, host = window.location.host) {
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    return `${protocol}//${host}${apiPrefix()}/live-chat/ws?token=${encodeURIComponent(isHosted() ? '' : token)}`;
  },
  unifiedWebSocketUrl(token: string, host = window.location.host) {
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    return `${protocol}//${host}${apiPrefix()}/unified-chat/ws?token=${encodeURIComponent(isHosted() ? '' : token)}`;
  },
  ...
  regenerateMessageUrl(sessionId: string, messageId: string | number) {
    return `${apiPrefix()}/chat/sessions/${encodeURIComponent(sessionId)}/messages/${encodeURIComponent(String(messageId))}/regenerate`;
  },
  ...
  sendThreadMessageUrl(threadId: string) {
    return `${apiPrefix()}/chat/threads/${encodeURIComponent(threadId)}/messages`;
  },
  listProjects() {
...
export const fileApi = {
  ...
  byNameUrl(filename: string) {
    return `${apiPrefix()}/files/content?filename=${encodeURIComponent(filename)}`;
  },
  contentUrl(attachmentId: string) {
    return `${apiPrefix()}/files/${encodeURIComponent(attachmentId)}/content`;
  },
  tokenUrl(fileToken: string) {
    return `${apiPrefix()}/files/tokens/${encodeURIComponent(fileToken)}`;
  },
};
...
export const pluginApi = {
  ...
  async installUpload(formData: FormData) {
    const response = await fetchWithAuth(`${apiPrefix()}/plugins/install/upload`, {
      method: 'POST',
      body: formData,
    });
