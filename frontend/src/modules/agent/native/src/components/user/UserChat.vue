<template>
  ...
  >
    ...
    <v-navigation-drawer
      ...
      :permanent="isChihiroHosted || lgAndUp"
      :temporary="!isChihiroHosted && !lgAndUp"
      :rail="!isChihiroHosted && lgAndUp && customizer.chatSidebarCollapsed"
      :width="280"
      :rail-width="56"
      location="left"
      floating
...
import { chatApi, providerApi } from "@/modules/agent/native/src/api/v1";
import StyledMenu from "@/modules/agent/native/src/components/shared/StyledMenu.vue";
import ProjectDialog, {
  type ProjectFormData,
} from "@/modules/agent/native/src/components/chat/ProjectDialog.vue";
import ProjectList, { type Project } from "@/modules/agent/native/src/components/chat/ProjectList.vue";
import ProjectView from "@/modules/agent/native/src/components/chat/ProjectView.vue";
import ChatInput from "@/modules/agent/native/src/components/chat/ChatInput.vue";
import ChatMessageList from "@/modules/agent/native/src/components/chat/ChatMessageList.vue";
import ChatUILogo from "@/modules/agent/native/src/components/chat/ChatUILogo.vue";
import type { RegenerateModelSelection } from "@/modules/agent/native/src/components/chat/RegenerateMenu.vue";
import ReasoningSidebar from "@/modules/agent/native/src/components/chat/ReasoningSidebar.vue";
import ThreadPanel from "@/modules/agent/native/src/components/chat/ThreadPanel.vue";
import WorkspaceFilesPanel from "@/modules/agent/native/src/components/chat/WorkspaceFilesPanel.vue";
import RefsSidebar from "@/modules/agent/native/src/components/chat/message_list_comps/RefsSidebar.vue";
import { useSessions, type Session } from "@/modules/agent/native/src/composables/useSessions";
import {
  ...
  type MessagePart,
  type TransportMode,
} from "@/modules/agent/native/src/composables/useMessages";
import { useMediaHandling } from "@/modules/agent/native/src/composables/useMediaHandling";
import { useRecording } from "@/modules/agent/native/src/composables/useRecording";
import { useProjects } from "@/modules/agent/native/src/composables/useProjects";
import { useDragUpload } from "@/modules/agent/native/src/composables/useDragUpload";
import { useChatHeaderStore } from "@/modules/agent/native/src/stores/chatHeader";
import { useCustomizerStore } from "@/modules/agent/native/src/stores/customizer";
import ProviderChatCompletionPanel from "@/modules/agent/native/src/components/provider/ProviderChatCompletionPanel.vue";
import {
  useI18n,
  useLanguageSwitcher,
  useModuleI18n,
} from "@/modules/agent/native/src/i18n/composables";
import type { Locale } from "@/modules/agent/native/src/i18n/types";
...
const chatSidebarDrawer = computed({
  get: () => isChihiroHosted.value || lgAndUp.value || customizer.chatSidebarOpen,
  set: (value: boolean) => {
    if (!lgAndUp.value) {
...
const isSidebarCollapsed = computed(() =>
  !isChihiroHosted.value && (lgAndUp.value ? customizer.chatSidebarCollapsed : !customizer.chatSidebarOpen),
);
...
.chat-main :deep(.provider-trigger--input) {
  height: 32px;
  max-width: 160px;
  padding: 0 8px;
  border: 0;
...
.chat-main :deep(.provider-trigger--input:hover) {
  border: 0;
}
